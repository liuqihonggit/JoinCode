   ...
   - 要条件链式命令：`A; if ($?) { B }`。无条件：`A; B`。";
        ...
        int maxOutputLength)
    {
        ...
        return string.Concat(
            ...
            $"    - 文件搜索：使用 {SearchToolNameConstants.Glob}（不是 Get-ChildItem -Recurse）\n",
            $"    - 内容搜索：使用 {SearchToolNameConstants.Grep}（不是 Select-String）\n",
            $"    - 读取文件：使用 {FileToolNameConstants.FileRead}（不是 Get-Content）\n",
            $"    - 编辑文件：使用 {FileToolNameConstants.FileEdit}\n",
            $"    - 写入文件：使用 {FileToolNameConstants.FileWrite}（不是 Set-Content/Out-File）\n",

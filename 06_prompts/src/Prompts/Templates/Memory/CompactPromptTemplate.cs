...
在适用的情况下包含文件名和代码片段。
9. 继续工作的上下文：摘要理解后续消息中的工作并继续工作所需的任何上下文、决策或状态。列出与你正在做的最近工作相关的下一步。
...
这应该是逐字的，以确保任务解释没有偏差。
10. 关键约束与决策：列出影响后续工作的已知约束（技术约束、业务约束、用户明确要求不做的事项）、已做出的关键决定（架构选型、方案取舍及理由，尤其是选 A 放弃 B 的决策）、以及正在依赖但未验证的重要假设。
11. 风险与注意事项：列出容易误判、重复劳动或跑偏的点；已经验证过且不建议继续的方向（避免重蹈覆辙）；已知的陷阱和踩过的坑。
12. 建议接手路径：告诉接手者应优先查看哪些文件、模块、日志、命令或线索；应先验证什么；推荐的下一步动作是什么。尽量引用具体路径、类名、命令。
...
9. 继续工作的上下文：
    [继续工作所需的关键上下文、决策或状态，以及相关的下一步]

10. 关键约束与决策：
    - [已知约束：技术/业务约束、用户要求不做的事项]
    - [已做出的关键决定：选 A 放弃 B，及理由]
    - [正在依赖但未验证的假设]
...
9. 继续工作的上下文：摘要理解后续消息中的工作并继续工作所需的任何上下文、决策或状态。列出与最近工作相关的下一步。包含最近对话中的直接引语。
10. 关键约束与决策：列出影响后续工作的已知约束、已做出的关键决定（尤其是选 A 放弃 B 的决策及理由）、以及正在依赖但未验证的重要假设。
11. 风险与注意事项：列出容易误判、重复劳动或跑偏的点；已经验证过且不建议继续的方向；已知的陷阱和踩过的坑。
12. 建议接手路径：告诉接手者应优先查看哪些文件、模块、日志、命令或线索；应先验证什么；推荐的下一步动作是什么。
...
10. 关键约束与决策：
    - [已知约束、已做出的关键决定及理由、未验证的假设]

11. 风险与注意事项：
    - [易误判/重复劳动的点、已验证不继续的方向、已知陷阱]

12. 建议接手路径：
    - [优先查看的文件/模块/命令、应先验证什么、推荐下一步]

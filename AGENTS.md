...
7. **代码风格：链式+有名函数** — 主逻辑用 LINQ 链式语法控制流程，不同部分提取为有名函数，调用处直观可见一切。或用 Actor 模型 / 管道中间件模型组织。禁止内联长 lambda 淹没主流程
8. **错误提示必须有诱导方式** — 安全拒绝/错误提示必须解释：① 为什么拒绝 ② 是什么参数/子命令触发的 ③ 接下来应该怎么做（正确写法）。禁止纯拒绝无引导，否则 AI 不知道错误含义会转用其他命令（禁止处发散，比安全策略更危险）
9. **提取重复为 LINQ 链式** — 发现代码重复部分必须提取为公共组件，并用 LINQ 链式语法组织。禁止散落重复逻辑，禁止复制粘贴
10. **硬编码变委托** — 硬编码字典/映射表/集合优先改为委托给统一数据源（如 `DangerousCommandCatalog`），消除多套不一致的定义。唯一数据源 + 委托消费，禁止双向维护
11. **目录层次化** — 文件夹按层次化组织：底层可看见上层，上层无法看见下层。通过构造 `private/` 目录包裹自己私有对象，提取公共部分放在上层文件夹。禁止跨层引用、禁止私有对象暴露到上层
...
| gh 工具 | 操作 PR/Issue/Release/CI 等 GitHub 资源时，优先用 `jcc gh` / `jcc mcp_call gh_*`；jcc 不可用时直接用系统 `gh` CLI > ADR: [0073](docs/adr/0073-gh-rest-api-direct-call.md)、[0089](docs/adr/0089-jcc-builtin-tools-only-no-system-gh-rg.md) |
| rg 工具 | 日常代码/文本搜索优先用 Agent 内置 `grep` 工具（`Grep` tool）；内置工具不可用时直接用系统 `rg` > ADR: [0070](docs/adr/0070-rg-engine-mmap-plinq.md) |
...
2. **📂 查项目代码** → .ps1 脚本、SearchCodebase、Agent 内置 `grep` 工具搜索现有实现模式（内置工具不可用时用系统 `rg`）
...
- jcc 报错或未安装时，直接用系统 `gh` / `rg` 可执行文件，不要卡住
...
- **优先**用 Agent 内置 `grep` 工具（`Grep` tool）进行代码/文本搜索
- 内置工具不可用时直接用系统 `rg`
...
| Workflow | 说明 |
|----------|------|
| `ci.yml` | 主入口，调用子 workflow |
| `ci-build.yml` | 七层有序编译 + 组件测试项目 + 卫星项目 |
| `ci-unit-tests.yml` | 每个 csproj 独立 matrix job，`--filter "Category!=Integration&Category!=Benchmark"` |
| `ci-integration.yml` | `Integration.Tests` + App.slnx filter 分组 |
| `ci-e2e.yml` | MockServer.E2E + Sync.Integration + CodeIndex.E2E + smoke tests |
| `mutation-testing.yml` | Stryker.NET，每日定时 |

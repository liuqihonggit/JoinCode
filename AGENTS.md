...
| `rg <pattern> <path> [path...]` | ripgrep 兼容搜索（**路径必填**，内置实现，复用 Grep 引擎） | `jcc rg "finally\s*\{" core/ --type cs -g "!**/tests/**" -n` |
...
5. **无匹配返回 1**（对齐 rg 退出码）
6. **二进制文件自动跳过，遵守 .gitignore**（复用 `SearchService` 现有逻辑）
7. **并行 + SIMD**：复用 `ISearchService.GrepSearchAsync`（`Task.WhenAll` 并行 + .NET Regex SIMD 引擎 + Span 零分配行遍历）

...
internal sealed class JccChatSession : IJccChatSession {
    ...
    public async Task SetEffortLevelAsync(EffortLevel effortLevel, CancellationToken cancellationToken = default) {
        ...
        if (configService is not null) {
            if (effortLevel is EffortLevel.Auto) {
                await configService.RemoveAsync(ConfigKeyEnumConstants.EffortLevel, cancellationToken).ConfigureAwait(false);
            } else {
                await configService.SetAsync(ConfigKeyEnumConstants.EffortLevel, effortLevel.ToValue(), cancellationToken).ConfigureAwait(false);
            }
        }
    }
    ...
    public async Task<ThemeKind> GetThemeAsync(CancellationToken cancellationToken = default) {
        ...
        var value = await configService.GetAsync(ConfigKeyEnumConstants.Theme, cancellationToken).ConfigureAwait(false);
    ...
    public async Task SetThemeAsync(ThemeKind theme, CancellationToken cancellationToken = default) {
        var configService = _services.GetService<IConfigurationService>();
        if (configService is not null)
            await configService.SetAsync(ConfigKeyEnumConstants.Theme, theme.ToValue(), cancellationToken).ConfigureAwait(false);
    }

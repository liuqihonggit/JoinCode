...
public sealed partial class MainViewModel {
    ...
    private async Task ReloadMessagesFromEngineAsync(ChatUiMessage echoToKeep) {
        try {
            var records = await _session.GetMessagesAsync(_sendCts?.Token ?? CancellationToken.None).ConfigureAwait(true);
            Messages.Clear();
    ...
    private async Task RegenerateLastReplyAsync() {
        ...
        await _session.RewindLastTurnAsync().ConfigureAwait(true);
    ...
    private async Task ClearHistoryInternalAsync() {
        Messages.Clear();
        await _session.ClearHistoryAsync().ConfigureAwait(true);
    }

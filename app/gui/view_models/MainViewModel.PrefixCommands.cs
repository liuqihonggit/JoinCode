...
public sealed partial class MainViewModel {
    ...
    private async Task HandleSilentPrefixCommandAsync(string message) {
        ...
        var result = await Cli.Commands.Prefix.PrefixCommandRouter.ExecuteAsync(message, context).ConfigureAwait(false);
        if (!result.Handled) {
    ...
    private async Task HandleShellPrefixCommandAsync(string message, System.Threading.CancellationToken ct) {
        ...
        var result = await Cli.Commands.Prefix.PrefixCommandRouter.ExecuteAsync(message, context, ct).ConfigureAwait(false);

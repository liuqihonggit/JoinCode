...
/// <summary>
/// MCP SSE 客户端 - 通过 SSE 接收消息，HTTP POST 发送消息
/// 桥接 SseClientTransport 传输层和 McpClientBase 客户端层
/// </summary>
public sealed partial class McpSseClient : McpClientBase
{
    private readonly McpServerConnectionConfig _config;
    private readonly Transports.SseClientTransport _transport;
        ...
        : base(options ?? new McpClientOptions(), logger)
    {
        ...
        _transport = new Transports.SseClientTransport(_config, resolvedAuthProvider, logger as ILogger<Transports.SseClientTransport>);
    ...
    public override async Task ConnectAsync(CancellationToken cancellationToken = default)
    {
        if (IsConnected)
        {
            _logger?.LogWarning("MCP SSE 客户端已连接");
            return;
        }

        _logger?.LogInformation("正在连接到 MCP SSE 服务器: {ServerName}", _config.Name);
        ...
        try
        {
            ...
            IsConnected = true;
            _logger?.LogInformation("MCP SSE 客户端连接成功");
        }
        catch (Exception ex)
        {
            _logger?.LogError(ex, "连接 MCP SSE 服务器失败");
    ...
    public override async Task DisconnectAsync(CancellationToken cancellationToken = default)
    {
        ...
        _logger?.LogInformation("正在断开 MCP SSE 客户端连接...");
        ...
        _logger?.LogInformation("MCP SSE 客户端已断开连接");
    }
    ...
    {
        _logger?.LogError(e.Exception, "SSE 传输错误");

...
public static class GoalGraphTemplates
{
    ...
    private static GoalGraph BuildCodeReviewGraph(GoalGraphEngine engine, string objective)
    {
        ...
        dag.AddNode(new DagNode<GoalNodePayload> { Id = "read", Payload = new() { Kind = GoalNodeKind.Agent, Name = "reader", AgentType = "reader", IsSubAgent = true, SystemPrompt = "You are a code reading expert. Thoroughly read and understand the code changes, identifying all modified files, functions, and logic.", Instruction = objective } });
        dag.AddNode(new DagNode<GoalNodePayload> { Id = "analyze", Payload = new() { Kind = GoalNodeKind.Agent, Name = "analyzer", AgentType = "analyzer", IsSubAgent = true, SystemPrompt = "You are an independent code review analyst. Evaluate code WITHOUT assuming context from the author. Check: correctness, security, performance, maintainability, error handling, edge cases.", Instruction = "Independently analyze the code for issues. Be thorough and objective.", FreshContext = true } });
        dag.AddNode(new DagNode<GoalNodePayload> { Id = "approve", Payload = new() { Kind = GoalNodeKind.Agent, Name = "approver", AgentType = "approver", IsSubAgent = true, SystemPrompt = "You are a review approver. The code has passed review. Summarize the review conclusion.", Instruction = "Summarize the approved review with key findings." } });
        dag.AddNode(new DagNode<GoalNodePayload> { Id = "suggest_fixes", Payload = new() { Kind = GoalNodeKind.Agent, Name = "fix-suggester", AgentType = "fix-suggester", IsSubAgent = true, SystemPrompt = "You are a code improvement expert. Based on the review findings, suggest specific fixes and improvements.", Instruction = "Suggest specific fixes based on the review findings." } });
    ...
    private static GoalGraph BuildTestGenGraph(GoalGraphEngine engine, string objective)
    {
        ...
        dag.AddNode(new DagNode<GoalNodePayload> { Id = "analyze", Payload = new() { Kind = GoalNodeKind.Agent, Name = "analyzer", AgentType = "analyzer", IsSubAgent = true, SystemPrompt = "You are a test analysis expert. Analyze the code to identify all testable behaviors, edge cases, and error paths.", Instruction = objective } });
        dag.AddNode(new DagNode<GoalNodePayload> { Id = "write_tests", Payload = new() { Kind = GoalNodeKind.Agent, Name = "test-writer", AgentType = "test-writer", IsSubAgent = true, SystemPrompt = "You are a test writing expert. Write comprehensive unit tests covering all identified behaviors. Follow TDD principles: arrange-act-assert, one assertion per concept.", Instruction = "Write comprehensive unit tests based on the analysis." } });
        dag.AddNode(new DagNode<GoalNodePayload> { Id = "run_tests", Payload = new() { Kind = GoalNodeKind.Agent, Name = "test-runner", AgentType = "test-runner", IsSubAgent = true, SystemPrompt = "You are an independent test verifier. Run the tests and verify they pass WITHOUT assuming context. Check coverage and correctness.", Instruction = "Run all tests and verify they pass. Check coverage.", FreshContext = true } });

...
- 使用 {AgentToolNameConstants.AgentSendMessage} 工具并设置 `to: "<name>"` 向特定团队成员发送消息
- 谨慎使用 {AgentToolNameConstants.AgentSendMessage} 工具并设置 `to: "*"` 进行团队范围的广播

仅在文本中编写回复对团队中的其他人不可见 - 你必须使用 {AgentToolNameConstants.AgentSendMessage} 工具。

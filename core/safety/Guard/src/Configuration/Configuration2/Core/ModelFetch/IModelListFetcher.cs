...
/// <summary>
/// 模型列表远程拉取器 — 从各供应商的 modelsEndpoint 并行拉取最新模型 id 列表
/// </summary>
public interface IModelListFetcher
{
    ...
    /// <returns>供应商名 → 远程模型 id 列表（拉取失败的供应商不包含在结果中）</returns>
    Task<IReadOnlyDictionary<string, IReadOnlyList<string>>> FetchAllAsync(

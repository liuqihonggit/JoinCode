namespace Core.Memdir;

/// <summary>
/// Memdir 组件配置选项
/// </summary>
[Register(typeof(MemdirOptions), ServiceLifetime.Singleton)]
public sealed partial class MemdirOptions : ServiceEntity
{
    /// <summary>
    /// 存储根路径
    /// </summary>
    public string StoragePath { get; set; } = Path.Combine(AppDataConstants.Paths.JccDirectory, "memdir");

    public MemdirOptions() { }

    /// <summary>
    /// DI 构造函数 — 从 WorkflowConfig 获取 StoragePath
    /// </summary>
    public MemdirOptions(WorkflowConfig? config)
    {
        StoragePath = config?.MemdirPath ?? Path.Combine(AppDataConstants.Paths.JccDirectory, "memdir");
    }
}

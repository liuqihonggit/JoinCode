...
ADR 0075 原计划新建 `DuplexActorBase<TIn, TOut>` 保持 `ActorBase` 不变。用户决策：项目不需要后向兼容，直接改 `ActorBase` 为双工，大修大改。
...
**替代方案（放弃）**：
- 新建 DuplexActorBase（ADR 0075）— 两套基类，维护负担

### 决策2：双 Channel + IAsyncEnumerable 输出

沿用 ADR 0075 的决策2-5（双 Channel、IAsyncEnumerable、输入有背压输出无背压、IAgentOutputChannelManager 集成）。
...
- [x] AsyncLock 70 个测试通过（排除预先存在的死锁检测 bug）

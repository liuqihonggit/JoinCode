...
### 提示语作废出口统一（2026-09-23）

ArgvHash 两轮确认的三个诊断（JCC9006 第一轮 / JCC9007 命令不匹配 / JCC9008 hash 不匹配）原提示语只引导 AI 二次确认，未明确告知 AI 可直接发新命令作废。AI 自省发现上轮输错后，可能困惑到硬二次确认错误命令。

**改动**：三个诊断统一增加「✅ 作废重发：直接输入新命令即可（将被视为新一轮首次确认，无需额外作废指令）」出口。无状态架构不变，不新增作废工具 —— 无状态下 AI 发新命令即隐式作废上一轮（被当新一轮第一轮处理），作废是隐式行为而非显式指令。

| 诊断 | 改动前 | 改动后 |
|------|--------|--------|
| JCC9006 | "请再次输入完全相同的命令…" | + ✅ 作废重发出口 |
| JCC9007 | "建议丢弃当前命令，完整重新生成" | ✅ 作废重发（明确视为新一轮首次确认）|
| JCC9008 | "建议丢弃当前命令，完整重新生成" | ✅ 作废重发（明确视为新一轮首次确认）|

67 + 8 测试通过。commit `6202ff474` + `004d38688`。

...
| M1 | 状态栏"还是差一点点"根治：模型徽章(Padding 9,2→高19)撑高主状态栏 3px — headless 测试漏了徽章显示场景；补 SelectedModel 场景红测试 → 两条状态栏固定 Height=30 + 内容收缩垂直居中同构（347 全绿） | ✅ |
...
<!-- 🤖 Auto Decision: 2026-08-25 -->
<!-- 决策: 两条状态栏固定 Height=30 + Padding 12,0 + 内容容器(Grid/StackPanel)统一 VerticalAlignment=Center 同构 -->
<!-- 原因: 用户两次反馈"差一点点"——第一层是 Padding/字号差异(11px+1px)，第二层是真实环境模型徽章(高19px)撑高主状态栏 3px 而 headless 测试 SelectedModel 为空徽章隐藏断言通过(测试盲区)；固定高度让徽章永远不撑行，同构 Center 让文字基线恒对齐 -->
<!-- 替代方案: 徽章缩小到文字行高（仍可能差 1px，不彻底）；测试补徽章场景保留（防回归）-->
<!-- 验证: StatusBar_HeightAligned 四重断言(等高/顶对齐/文字基线/徽章场景)全绿，347 测试通过 ✅ -->

...
| 维度 | 检查项 |
|------|--------|
| 需求可测试性 | 每条 EARS 都有明确触发条件 + 可观察结果，无模糊措辞 |
| 依赖无环 | P0-P5 优先级不存在循环依赖（②是底座，已被所有上层依赖） |
| 架构兼容 | 新增模块落点不破坏七层编译顺序（Foundation→Core→Services） |
| 资产复用 | 可复用资产（循环干预状态机/截图链路/Guard）已被识别并标注 |
| 非目标清晰 | 明确不做什么（不替换 CLI、不做移动端、不引入非 AOT SDK） |

### 6.2 能力建设验收方法

核心思路：**真实 `jcc.exe` + 真实受控桌面应用 + 断言最终状态**，不模拟对话（遵循项目 E2E 理念）。

- 用 `Start-Process` 启动 jcc.exe（带 `--await` 超时兜底，超时返回 1234）
- 操作真实桌面应用（记事本/计算器/资源管理器）
- 读取文件/窗口状态做断言
- 失败则读 `dumps/crash_*.log` 定位（`AppDomain.UnhandledException` 挂钩）

### 6.3 里程碑验收场景

| 里程碑 | 验收场景 | 断言 |
|--------|---------|------|
| M1（P0 输入模拟） | jcc 接收"在记事本里输入 hello world 并保存到 X.txt" | 文件 X.txt 内容 == "hello world" |
| M2（P1 视觉理解） | jcc 接收"截图计算器，找到数字 7 按钮并点击" | 计算器显示框出现 "7" |
| M3（P2 环境感知） | jcc 操作触发保存覆盖弹窗 → 自主选择"否" | 原文件未被覆盖，操作继续 |
| M3（撤销元意识） | jcc 被要求删除文件 → 执行前自动备份 | 备份存在 + 删除后可恢复 |
| M4（P3 复合链） | jcc 从浏览器拖文本到记事本 | 记事本含该文本 |
| M5（P4 宏录制） | 录制"重命名 10 个文件" → 回放 | 10 个文件名按规则变更 |
| M6（P5 观察学习） | 演示一次"配置 VPN" → 新环境复现 | 关键配置项一致 |

### 6.4 各里程碑能力 Checklist

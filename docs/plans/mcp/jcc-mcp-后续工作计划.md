# jcc mcp 扁平元动词子命令后续工作计划

> 关联 ADR: [0069](../../adr/0069-cli-args-full-refactor.md)（supersede [0065](../../adr/0065-jcc-mcp-subcommand.md)）
> 创建日期: 2026-09-05

## 已完成

- ✅ `jcc mcp_call/mcp_list/mcp_search/mcp_schema` 四个扁平元动词子命令（ADR 0069 重构）
- ✅ `jcc mcp_serve` 子命令 — stdio + http 双模式，暴露 387 个工具
...
- ✅ jcc mcp_search/mcp_list --json AOT 序列化 bug 修复 — 匿名类型改用具名 record（commit 91c31fbf8）
...
- [ ] 创建新 PR 包含 jcc mcp 扁平元动词子命令 + mcp_serve + bug 修复
- [ ] auto-merge 合并到 main

### P5 — 文档

- [ ] README 加 `jcc mcp_call/mcp_list/mcp_search/mcp_schema/mcp_serve` 子命令用法章节（中英文）
- [ ] AGENTS.md 加 `jcc mcp_serve` 启动测试说明（MockServer 联调）
- [ ] ADR 0066 — jcc mcp_serve 设计决策（如需独立 ADR）

## 风险

- **serve 启动开销**：构建完整 DI 容器需 1-2 秒（387 个工具注册），高频调用场景应复用 serve 服务端而非每次 `jcc mcp_call`
...
<!-- 决策: jcc mcp_serve 用 JccMcpServer 继承 McpServer 而非包装器 -->

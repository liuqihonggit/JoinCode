...
/// <summary>
/// Bedrock 模型辅助工具 — 对齐 claude code src/utils/model/bedrock.ts
...
{
    /// <summary>
    /// Bedrock 跨区域推理前缀列表 — 对齐 claude code BEDROCK_REGION_PREFIXES
    /// <para>us: 美国区域, eu: 欧洲区域, apac: 亚太区域, global: 全球</para>
    /// </summary>
    public static readonly string[] RegionPrefixes = ["us", "eu", "apac", "global"];

    /// <summary>
    /// 判断模型 ID 是否是 Bedrock foundation model — 对齐 claude code isFoundationModel
    ...
    /// <summary>
    /// 从 Bedrock ARN 提取模型/推理配置 ID — 对齐 claude code extractModelIdFromArn
    ...
    /// <summary>
    /// 从 Bedrock 跨区域推理模型 ID 提取区域前缀 — 对齐 claude code getBedrockRegionPrefix
    ...
    /// <summary>
    /// 给 Bedrock 模型 ID 应用区域前缀 — 对齐 claude code applyBedrockRegionPrefix
    ...
    /// <summary>
    /// 应用父级区域前缀到子代理模型 — 对齐 claude code getAgentModel.applyParentRegionPrefix

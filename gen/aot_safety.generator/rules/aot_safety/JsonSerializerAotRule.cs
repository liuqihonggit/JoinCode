...
/// <summary>
/// JCC1011/JCC1012: JSON 序列化 AOT 安全。多描述符规则。
/// JCC1011: JsonSerializer 调用缺少 JsonTypeInfo 参数。
/// JCC1012: JsonSerializerOptions 未设置 TypeInfoResolver。测试项目豁免。
/// </summary>
[AnalyzerRule(
    AnalyzerId = "JsonSerializerAot",
    Id = "JCC1011",
    Title = "AOT incompatible: JsonSerializer call missing JsonTypeInfo parameter",
    Description = "JsonSerializer.{0}() 调用缺少 JsonTypeInfo 参数，在 NativeAOT 下会触发反射序列化异常。应使用 JsonSerializer.{0}(value, JsonContext.Default.T) 重载。",
    Category = "AotSafety",
...
public sealed class JsonSerializerAotRule : IAnalyzerRule {
    ...
    private static bool UsesOptionsWithResolver(InvocationExpressionSyntax invocation, SyntaxNodeAnalysisContext ctx) {
        foreach (var arg in invocation.ArgumentList.Arguments) {
            ...
            if (argSymbol is ILocalSymbol local && IsJsonSerializerOptionsType(local.Type))
                return true;
            if (argSymbol is IParameterSymbol param && IsJsonSerializerOptionsType(param.Type))
                return true;
            if (argSymbol is IFieldSymbol field && IsJsonSerializerOptionsType(field.Type))
                return true;
        }
        return false;
    }
    ...
    private static bool HasTypeInfoResolverInitializer(ObjectCreationExpressionSyntax creation) {
        foreach (var initializer in creation.Initializer?.Expressions ?? []) {
            if (initializer is AssignmentExpressionSyntax assignment && assignment.Left.ToString().Trim() == "TypeInfoResolver")
                return true;
        }
        return false;
    }

...
/// <summary>
/// JCC11001: 容器初始化 — 容器类型字段/属性必须初始化，禁止为 null。
/// 可空容器（如 List of T 问号）允许，用于延迟初始化场景。
/// 构造函数中赋值的字段/属性自动豁免（按名字匹配，类型级计算一次）。
/// </summary>
[AnalyzerRule(
    AnalyzerId = "ContainerInitialization",
    Id = "JCC11001",
    Title = "容器初始化: 容器类型字段/属性必须初始化，禁止为 null",
    Description = "容器类型 {0} '{1}' 未初始化，默认为 null。应在声明时初始化（如 = new()），或显式标记为可空。未初始化的容器在使用时会导致 NullReferenceException。",
    Category = "CodeStyle",
    Severity = DiagnosticSeverity.Warning,
    IsEnabledByDefault = true,
    HelpLinkUri = "容器类型（List/Dictionary/HashSet/数组等）未初始化时默认为 null, 使用时抛 NullReferenceException. 合规写法: 1) 声明时初始化; 2) 显式标记可空; 3) 构造函数赋值（已自动识别豁免）. 可空容器允许, 用于延迟初始化场景.")]
public sealed class ContainerNotInitializedRule : AnalyzerRuleBase<ContainerNotInitializedRule> {
    ...
    private static void AnalyzeFieldDeclaration(SyntaxNodeAnalysisContext ctx, HashSet<string> assignedNames) {
        ...
        foreach (var declarator in fieldDecl.Declaration.Variables) {
            ...
            ctx.ReportDiagnostic(Diagnostic.Create(Descriptor, declarator.GetLocation(), typeName, fieldName));
        }
    }
    ...
    private static void AnalyzePropertyDeclaration(SyntaxNodeAnalysisContext ctx, HashSet<string> assignedNames) {
        ...
        ctx.ReportDiagnostic(Diagnostic.Create(Descriptor, propDecl.Identifier.GetLocation(), typeName, propName));
    }

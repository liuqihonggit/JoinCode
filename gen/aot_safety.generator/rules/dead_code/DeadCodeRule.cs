...
/// <summary>
/// JCC7001/JCC7002/JCC7003: 死代码检测。多描述符规则 — 方法/类型/枚举未被引用。
/// </summary>
[AnalyzerRule(
    AnalyzerId = "DeadCode",
    Id = "JCC7001",
    Title = "死代码: 方法 '{0}' 未被任何代码引用",
    Description = "方法 '{0}' 在当前编译中未被引用。请先分析是否为分析器盲区（如源码生成器调用、事件订阅、方法组引用等），若是则修复分析器屏蔽规则；确认非误报后再考虑移除或使用 #pragma warning disable JCC7001 抑制。",
    Category = "DeadCodeAudit",
    Severity = DiagnosticSeverity.Info,
    IsEnabledByDefault = true,
    HelpLinkUri = "未被引用的 private/internal 方法可能是遗留代码或分析器盲区. 优先修复分析器屏蔽规则而非抑制警告. 接口实现、重写方法、序列化构造函数、源码生成器特性标注、事件订阅等自动排除.",
    IsCompilationEnd = true)]
[AnalyzerRule(
    AnalyzerId = "DeadCode",
    Id = "JCC7002",
    Title = "死代码: 类型 '{0}' 未被任何代码引用",
    Description = "类型 '{0}' 在当前编译中未被引用。请先分析是否为分析器盲区，若是则修复分析器屏蔽规则；确认非误报后再考虑移除或使用 #pragma warning disable JCC7002 抑制。",
    Category = "DeadCodeAudit",
    Severity = DiagnosticSeverity.Info,
    IsEnabledByDefault = true,
    HelpLinkUri = "未被引用的 internal 类型可能是遗留代码或分析器盲区. 优先修复分析器屏蔽规则而非抑制警告. 含 public 成员的类型自动排除.",
    IsCompilationEnd = true)]
[AnalyzerRule(
    AnalyzerId = "DeadCode",
    Id = "JCC7003",
    Title = "死代码: 枚举 '{0}' 未被任何代码引用",
    Description = "枚举 '{0}' 在当前编译中未被引用。请先分析是否为分析器盲区，若是则修复分析器屏蔽规则；确认非误报后再考虑移除或使用 #pragma warning disable JCC7003 抑制。",
    Category = "DeadCodeAudit",
    Severity = DiagnosticSeverity.Info,
    IsEnabledByDefault = true,
    HelpLinkUri = "未被引用的 internal 枚举可能是遗留代码或分析器盲区. 优先修复分析器屏蔽规则而非抑制警告. 含 [Flags] 特性的枚举自动排除.",
    IsCompilationEnd = true)]
public sealed class DeadCodeRule : IAnalyzerRule {
    private static readonly IReadOnlyDictionary<string, DiagnosticDescriptor> Map = RuleDescriptorFactory.CreateAll<DeadCodeRule>();
    ...
    public void Register(CompilationStartAnalysisContext context, ProjectContext projectContext) {
        ...
        context.RegisterSyntaxNodeAction(ctx => {
            ...
            if (symbol is null) return;
            if (symbol.DeclaredAccessibility != Accessibility.Internal) return;
        ...
        context.RegisterCompilationEndAction(ctx => {
            ...
            foreach (var kvp in methodDeclarations) {
                ...
                if (symbol.GetAttributes().Any(attr => attr.AttributeClass is not null && attr.AttributeClass.Name == "ModuleInitializerAttribute")) continue;
                if (symbol.ExplicitInterfaceImplementations.Length > 0) continue;
                if (syntax.ExplicitInterfaceSpecifier is not null) continue;
                if (IsInterfaceImplementation(symbol)) continue;
                if (symbol.IsOverride) continue;
                if (symbol.MethodKind is MethodKind.UserDefinedOperator or MethodKind.PropertyGet or MethodKind.PropertySet or MethodKind.EventAdd or MethodKind.EventRemove or MethodKind.Constructor or MethodKind.StaticConstructor or MethodKind.Destructor) continue;
                if ((symbol.DeclaredAccessibility == Accessibility.Internal || symbol.DeclaredAccessibility == Accessibility.ProtectedAndInternal) && HasInternalsVisibleTo(symbol.ContainingAssembly)) continue;
                ...
                ctx.ReportDiagnostic(Diagnostic.Create(Map["JCC7001"], syntax.Identifier.GetLocation(), methodName));
            }
            ...
            foreach (var kvp in typeDeclarations) {
                ...
                if ((symbol.DeclaredAccessibility == Accessibility.Internal || symbol.DeclaredAccessibility == Accessibility.ProtectedAndInternal) && HasInternalsVisibleTo(symbol.ContainingAssembly)) continue;
                if (referencedTypes.ContainsKey(symbol)) continue;
                var typeNameStr = symbol.ToDisplayString(SymbolDisplayFormat.MinimallyQualifiedFormat);
                ctx.ReportDiagnostic(Diagnostic.Create(Map["JCC7002"], syntax.Identifier.GetLocation(), typeNameStr));
            }
            ...
            foreach (var kvp in enumDeclarations) {
                ...
                ctx.ReportDiagnostic(Diagnostic.Create(Map["JCC7003"], syntax.Identifier.GetLocation(), enumName));
            }
        });
    }
    ...
    private static bool IsInterfaceImplementation(IMethodSymbol method) {
        ...
        foreach (var iface in containingType.AllInterfaces) {
            foreach (var ifaceMember in iface.GetMembers(method.Name)) {
                if (ifaceMember is IMethodSymbol ifaceMethod && method.Parameters.Length == ifaceMethod.Parameters.Length) {
    ...
    private static bool HasInternalsVisibleTo(IAssemblySymbol? assembly) {
        ...
        foreach (var attr in assembly.GetAttributes()) {
            if (attr.AttributeClass is not null && attr.AttributeClass.Name == "InternalsVisibleToAttribute")
                return true;
        }
        return false;
    }
    ...
    private static bool IsMethodReferenced(IMethodSymbol method, ConcurrentDictionary<IMethodSymbol, byte> referencedMethods) {
        ...
        if (method.IsGenericMethod && !method.IsDefinition && referencedMethods.ContainsKey(method.ConstructedFrom)) return true;
        return false;
    }
    ...
    private static bool HasCustomAttribute(IMethodSymbol method) {
        foreach (var attr in method.GetAttributes()) {
            ...
            if (ns is not null && ns.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat).StartsWith("global::System", StringComparison.Ordinal))
                continue;
            return true;
        }
        return false;

...
public sealed class CommandRegistrationGenerator : IIncrementalGenerator
{
    ...
    private static void GenerateSlashCommandCatalog(SourceProductionContext context, ImmutableArray<CommandInfo> commands)
    {
        ...
        sb.AppendLine("    /// </summary>");
        sb.AppendLine("    private static readonly FrozenDictionary<string, IReadOnlyList<SlashCommandMetadata>> _byCategory =");
        sb.AppendLine("        new Dictionary<string, IReadOnlyList<SlashCommandMetadata>>(StringComparer.OrdinalIgnoreCase)");
        sb.AppendLine("        {");
    ...
    private static void GenerateSlashCommandSchemaCatalog(SourceProductionContext context, ImmutableArray<CommandInfo> commands)
    {
        ...
        sb.AppendLine("{");
        sb.AppendLine("    private static readonly FrozenDictionary<string, SlashCommandSchemaEntry> _entryByName =");
        ...
        sb.AppendLine();
        sb.AppendLine("    /// <summary>");
        sb.AppendLine("    /// 按命令名查找完整 schema 条目(含 ArgumentHint),找不到返回 null。O(1) 字典查找。");
        sb.AppendLine("    /// </summary>");
        sb.AppendLine("    /// <param name=\"commandName\">命令名(不含斜杠前缀)。</param>");
        sb.AppendLine("    /// <returns>匹配的 schema 条目;若命令不存在返回 null。</returns>");
        sb.AppendLine("    public SlashCommandSchemaEntry? GetEntry(string commandName) =>");
        sb.AppendLine("        _entryByName.TryGetValue(commandName, out var entry) ? entry : null;");
        sb.AppendLine();
        sb.AppendLine("    /// <summary>");
        sb.AppendLine("    /// 按命令名查找参数 schema,找不到返回 null。O(1) 字典查找。");
        sb.AppendLine("    /// </summary>");
        sb.AppendLine("    /// <param name=\"commandName\">命令名(不含斜杠前缀)。</param>");
        ...
        sb.AppendLine("        _entryByName.TryGetValue(commandName, out var entry) ? entry.Schema : null;");
        sb.AppendLine("}");

...
public sealed class KeybindingsSkill
{
    ...
    public static SkillDefinition CreateDefinition()
    {
        return new SkillDefinition
        {
            ...
            {
                new() { Id = "read_keybindings", Type = SkillStepType.Tool, Tool = FileToolNameEnumConstants.FileRead, Description = "读取现有键绑定文件", Prompt = "读取 ~/.jcc/keybindings.json。如果文件不存在，说明需要创建新文件。", Next = "analyze" },
                ...
                new() { Id = "apply", Type = SkillStepType.Tool, Tool = FileToolNameEnumConstants.FileEdit, Description = "应用键绑定变更", Prompt = "根据操作类型修改 keybindings.json：\n\n- rebind: 解绑旧键 + 绑定新键\n- unbind: 将键设为 null\n- add: 添加新绑定\n- list: 仅展示，不修改\n- reset: 删除用户自定义，恢复默认\n\n确保 JSON 格式正确，保留现有绑定。", Next = "verify" },

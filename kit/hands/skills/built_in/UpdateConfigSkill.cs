...
public sealed class UpdateConfigSkill {
    ...
    public static SkillDefinition CreateDefinition() {
        return new SkillDefinition {
            ...
            {
                new() { Id = "read_config", Type = SkillStepType.Tool, Tool = FileToolNameEnumConstants.FileRead, Description = "读取现有配置文件", Prompt = "读取 {{target_file}} 的内容。如果文件不存在，说明需要创建新文件。", Next = "analyze" },
                new() { Id = "analyze", Type = SkillStepType.Prompt, Description = "分析现有配置和请求变更", Prompt = "分析现有配置文件内容，确定如何应用变更：\n\n目标文件：{{target_file}}\n配置路径：{{setting_path}}\n新值：{{value}}\n合并模式：{{merge}}\n\n请确定：\n1. 现有配置结构\n2. 需要修改的具体位置\n3. 是否需要合并数组（如权限列表）\n4. 是否会覆盖现有设置", Next = "apply_change" },
                new() { Id = "apply_change", Type = SkillStepType.Tool, Tool = FileToolNameEnumConstants.FileEdit, Description = "应用配置变更", Prompt = "使用 Edit 工具修改 {{target_file}}：\n\n配置路径：{{setting_path}}\n新值：{{value}}\n\n注意：\n- 保持 JSON 格式正确\n- 合并数组时追加而非替换\n- 保留现有配置项", Next = "validate" },

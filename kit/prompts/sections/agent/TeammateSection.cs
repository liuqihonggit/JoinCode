...
public static class TeammateSection {
    ...
    /// 团队成员系统提示词附加内容
    /// </summary>
    public static string TeammateSystemPromptAddendum { get; } = $"""
# 代理团队成员通信

重要：你正在作为团队中的代理运行。要与团队中的任何人通信：
- 使用 {AgentToolNameEnumConstants.AgentSendMessage} 工具并设置 `to: "<name>"` 向特定团队成员发送消息
- 谨慎使用 {AgentToolNameEnumConstants.AgentSendMessage} 工具并设置 `to: "*"` 进行团队范围的广播

仅在文本中编写回复对团队中的其他人不可见 - 你必须使用 {AgentToolNameEnumConstants.AgentSendMessage} 工具。

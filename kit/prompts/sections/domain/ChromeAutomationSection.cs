...
""";
    ...
    /// Chrome 工具搜索指令
    /// </summary>
    public static readonly string ChromeToolSearchInstructions = $"""
**重要：在使用任何 Chrome 浏览器工具之前，你必须首先使用 {SystemToolNameEnumConstants.ToolSearch} 加载它们。**

Chrome 浏览器工具是需要在使用前加载的 MCP 工具。在调用任何 mcp__claude-in-chrome__* 工具之前：
1. 使用 {SystemToolNameEnumConstants.ToolSearch} 并带上 `select:mcp__claude-in-chrome__<tool_name>` 加载特定工具
2. 然后调用该工具

例如，要获取标签页上下文：
1. 首先：{SystemToolNameEnumConstants.ToolSearch} 查询 "select:mcp__claude-in-chrome__tabs_context_mcp"
...
通过调用 {SkillToolNameEnumConstants.Skill} 工具并设置 skill: "claude-in-chrome" 来调用该技能。
该技能提供浏览器自动化说明并启用工具。
""";
...
—— 在任何 mcp__claude-in-chrome__* 工具之前调用 {SkillToolNameEnumConstants.Skill}(skill: "claude-in-chrome")。
""";

...
可用工具：{FileToolNameEnumConstants.FileRead}、{SearchToolNameEnumConstants.Grep}、{SearchToolNameEnumConstants.Glob}、只读 {ShellToolNameEnumConstants.Bash}（ls/find/cat/stat/wc/head/tail 等类似命令），以及仅用于记忆目录内路径的 {FileToolNameEnumConstants.FileEdit}/{FileToolNameEnumConstants.FileWrite}。不允许使用 {ShellToolNameEnumConstants.Bash} rm。所有其他工具 - MCP、{AgentToolNameEnumConstants.Agent}、可写入 {ShellToolNameEnumConstants.Bash} 等 - 将被拒绝。

你的回合预算有限。{FileToolNameEnumConstants.FileEdit} 需要事先对同一文件进行 {FileToolNameEnumConstants.FileRead}，因此高效策略是：第 1 回合 - 并行发出所有 {FileToolNameEnumConstants.FileRead} 调用，读取你可能更新的每个文件；第 2 回合 - 并行发出所有 {FileToolNameEnumConstants.FileWrite}/{FileToolNameEnumConstants.FileEdit} 调用。不要在多个回合中交错读取和写入。

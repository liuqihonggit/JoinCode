...
你唯一的任务是使用 {FileToolNameEnumConstants.FileEdit} 工具更新文档文件（如果有大量新信息要添加），然后停止。你可以进行多次编辑（根据需要更新多个部分）- 在单个消息中并行进行所有 {FileToolNameEnumConstants.FileEdit} 工具调用。如果没有大量内容要添加，只需用简要说明回复，不要调用任何工具。
...
使用 {FileToolNameEnumConstants.FileEdit} 工具，file_path: {docPath}

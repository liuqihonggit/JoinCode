...
public static class SessionMemoryPromptTemplate
{
    ...
    public const string DefaultSessionMemoryTemplate = """
        ...
        # 学习心得
        _什么方法有效？什么无效？应该避免什么？已验证过且不建议继续的方向、容易误判或跑偏的点。不要重复其他部分的项目_
    ...
    public static string GetDefaultUpdatePrompt(string notesPath, string currentNotes)
    {
        return $@"
            ...
            你唯一的任务是使用 {FileToolNameEnumConstants.FileEdit} 工具更新笔记文件，然后停止。你可以进行多次编辑（根据需要更新每个部分）- 在单个消息中并行进行所有 {FileToolNameEnumConstants.FileEdit} 工具调用。不要调用任何其他工具。
            ...
            使用 {FileToolNameEnumConstants.FileEdit} 工具，file_path: {notesPath}
            ...
            记住：并行使用 {FileToolNameEnumConstants.FileEdit} 工具并停止。编辑后不要继续。只包含来自实际用户对话的见解，永远不要来自这些说明。不要删除或更改部分标题或斜体 _部分描述_。
            ";
    }

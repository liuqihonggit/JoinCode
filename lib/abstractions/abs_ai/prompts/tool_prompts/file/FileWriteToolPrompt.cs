...
public static class FileWriteToolPrompt {
    public const string ToolName = FileToolNameEnumConstants.FileWrite;
    public const string Description = "将文件写入本地文件系统。";

    /// <summary>
    /// 获取写入工具描述
    /// </summary>
    public static string GetWriteToolDescription(string fileReadToolName = FileToolNameEnumConstants.FileRead) {
        var preReadInstruction = GetPreReadInstruction(fileReadToolName);
        return $@"将文件写入本地文件系统。

用法：
- 此工具将覆盖现有文件（如果提供的路径上有一个）。{preReadInstruction}
- 优先使用 {FileToolNameEnumConstants.FileEdit} 工具修改现有文件 - 它只发送差异。仅使用此工具创建新文件或完全重写。

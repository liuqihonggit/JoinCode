...
在计划模式中，你将：
1. 使用 {SearchToolNameEnumConstants.Glob}、{SearchToolNameEnumConstants.Grep} 和 {FileToolNameEnumConstants.FileRead} 工具彻底探索代码库
2. 理解现有模式和架构
3. 设计实现方法
4. 向用户展示你的计划以获得批准
...
- 添加具有明确要求的一个函数
- 用户给出了非常具体、详细说明的任务
- 纯研究/探索任务（改用 {AgentToolNameEnumConstants.Agent} 工具并选择 explore 代理）
...
- 一旦理解错误，修复就很清楚的错误修复
- 研究/探索任务（改用 {AgentToolNameEnumConstants.Agent} 工具）

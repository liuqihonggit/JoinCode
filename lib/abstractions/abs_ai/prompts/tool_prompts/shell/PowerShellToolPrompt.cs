...
public static class PowerShellToolPrompt {
    public const string ToolName = ShellToolNameEnumConstants.Powershell;
   ...
   - 要条件链式命令：`A; if ($?) { B }`。无条件：`A; B`。";
    ...
    private static string BuildMainPromptTemplate(
        ...
        return string.Concat(
            ...
            $"    - 文件搜索：使用 {SearchToolNameEnumConstants.Glob}（不是 Get-ChildItem -Recurse）\n",
            $"    - 内容搜索：使用 {SearchToolNameEnumConstants.Grep}（不是 Select-String）\n",
            $"    - 读取文件：使用 {FileToolNameEnumConstants.FileRead}（不是 Get-Content）\n",
            $"    - 编辑文件：使用 {FileToolNameEnumConstants.FileEdit}\n",
            $"    - 写入文件：使用 {FileToolNameEnumConstants.FileWrite}（不是 Set-Content/Out-File）\n",

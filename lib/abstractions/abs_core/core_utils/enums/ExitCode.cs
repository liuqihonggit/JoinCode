...
public enum ExitCode
{
    ...
    [EnumValue("general_error")] GeneralError = 1,

    /// <summary>配置错误（ConfigurationException）— 配置文件/环境变量问题，用户可自行修复</summary>
    [EnumValue("configuration_error")] ConfigurationError = 2,

    /// <summary>参数解析错误 — 命令行参数格式无效</summary>
    [EnumValue("argument_parse_error")] ArgumentParseError = 3,

    /// <summary>API Key 缺失或无效 — 需要配置认证信息</summary>
    [EnumValue("api_key_missing")] ApiKeyMissing = 4,

    /// <summary>会话恢复失败 — 会话文件损坏或不存在</summary>
    [EnumValue("session_resume_failed")] SessionResumeFailed = 5,

    // ── 10-19: 运行时错误 ──

    /// <summary>LLM 调用失败 — API 请求错误、速率限制、模型不可用等</summary>
    [EnumValue("llm_call_failed")] LlmCallFailed = 10,

    /// <summary>工具执行失败 — 工具调用抛出异常或返回错误</summary>
    [EnumValue("tool_execution_failed")] ToolExecutionFailed = 11,

    /// <summary>MCP 连接失败 — MCP 服务器连接/握手失败</summary>
    [EnumValue("mcp_connection_failed")] McpConnectionFailed = 12,

    /// <summary>子进程异常退出 — 病人进程/Bridge 子进程崩溃</summary>
    [EnumValue("subprocess_crashed")] SubprocessCrashed = 13,

    // ── 12xx: 超时与卡死（按类型细分，看到退出码即可定位是哪种超时） ──

    /// <summary>--await 诊断超时强制退出 — 历史契约值，CI/E2E 脚本依赖此值</summary>
    [EnumValue("await_timeout")] AwaitTimeout = 1234,

    /// <summary>LLM 调用超时 — API 请求超过时限未响应</summary>
    [EnumValue("llm_call_timeout")] LlmCallTimeout = 1240,

    /// <summary>工具执行超时 — 工具运行超过时限（如 Bash 命令卡死）</summary>
    [EnumValue("tool_execution_timeout")] ToolExecutionTimeout = 1241,

    /// <summary>MCP 连接超时 — MCP 服务器连接/握手超过时限</summary>
    [EnumValue("mcp_connection_timeout")] McpConnectionTimeout = 1242,

    /// <summary>子进程超时 — 病人/Bridge 子进程超过时限未退出</summary>
    [EnumValue("subprocess_timeout")] SubprocessTimeout = 1243,

    /// <summary>流式响应超时 — SSE/streaming 连接超过时限无数据</summary>
    [EnumValue("stream_response_timeout")] StreamResponseTimeout = 1244,

    // ── 128+N: 信号终止（POSIX 约定） ──

    /// <summary>用户中断 Ctrl+C（128 + SIGINT=2）— POSIX 标准，便于 shell 脚本区分中断与正常错误</summary>
    [EnumValue("interrupted")] Interrupted = 130,
}

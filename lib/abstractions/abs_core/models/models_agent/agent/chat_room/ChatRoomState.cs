...
/// <para>P2-⑪ 双索引合并: 原 Messages(Dictionary)+MessagesByTime(排序列表) 合并为单一 Messages 字典,按需排序。</para>
/// </summary>
public sealed record ChatRoomState {
    ...
    /// <summary>消息字典（MessageId → Message），唯一数据源 — O(1) 去重检查 + O(1) 按 MessageId 查找,按需排序遍历。</summary>
    ...
    /// <summary>最后消息时间（null 表示无消息）— O(n) 遍历 Messages.Values 取 Max(Timestamp)</summary>
    public DateTime? LastMessageAt => Messages.Count > 0 ? Messages.Values.Max(static m => m.Timestamp) : null;
    ...
    public (ChatRoomState State, bool Added) TryAddMessage(TeamMessage message) {
        ...
        return (this with { Messages = Messages.Add(message.MessageId, message) }, true);
    }

    /// <summary>
    /// 替换消息（撤回场景）— O(1) 按 MessageId 查找并替换。
    /// </summary>
    ...
    public ChatRoomState ReplaceMessage(string messageId, TeamMessage newMessage) {
        ...
        return this with { Messages = Messages.SetItem(messageId, newMessage) };
    }

    /// <summary>
    /// 批量设置消息（持久化加载）— O(n) 构建 Messages 字典。
    /// </summary>
    public ChatRoomState WithMessages(IEnumerable<TeamMessage> messages) {
        var byId = messages.ToImmutableDictionary(m => m.MessageId);
        return this with { Messages = byId };
    }

    /// <summary>
    /// 清理旧消息 — 删除超过 <see cref="MaxMessageCount"/> 的最旧消息。O(n log n) 排序后取前 N 个。

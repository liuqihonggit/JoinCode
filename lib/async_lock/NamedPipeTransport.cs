    ...
    private async Task AcceptConnectionsLoopAsync(CancellationToken ct)
    {
        while (!_cts.IsCancellationRequested && !ct.IsCancellationRequested)
        {
            ...
            try
            {
                ...
                _ = Task.Run(() => HandleHostConnectionAsync(server, ct), ct);
            }
            catch (OperationCanceledException) { return; }
            catch (Exception ex)
            {
                _logger?.LogError(ex, "NamedPipeTransport: accept connection error");
            }
        }
    }
...
/// <summary>管道连接 — 封装单个从机连接的读写。</summary>
internal sealed class PipeConnection : IAsyncDisposable
{
    ...
    public async ValueTask WriteAsync(ReadOnlyMemory<byte> data, CancellationToken ct)
    {
        ...
        using var guard = (await _writeLock.TryLockAsync(ct).ConfigureAwait(false))
            ?? throw new TimeoutException("PipeConnection: write lock timeout");
        try
        {

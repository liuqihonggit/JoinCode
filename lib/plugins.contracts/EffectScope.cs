...
public sealed class EffectScope : IAsyncDisposable {
    ...
    private async ValueTask DisposeAsyncChainAsync() {
        while (_asyncUndo.Count > 0) {
            ...
            try { await d.DisposeAsync().ConfigureAwait(false); } catch (Exception ex) { ReportRevertFailed(ex, "异步撤销"); }
        }
    }
    ...
    private void DisposeSyncChain() {
        while (_undo.Count > 0) {
            ...
            try { revert(); } catch (Exception ex) { ReportRevertFailed(ex, desc); }
        }
    }

    private void ReportRevertFailed(Exception ex, string? desc) {
        try { _onRevertFailed?.Invoke(ex, desc); } catch (Exception callbackEx) {

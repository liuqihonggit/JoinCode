...
/// <summary>
/// AdvisorService fire-and-forget CancellationToken 保护测试
/// 验证 Dispose 后取消令牌传播，以及已取消令牌不会导致崩溃
/// </summary>
public sealed class AdvisorServicePersistenceTests
{
    [Fact]
    ...
    public async Task PersistAsync_WithCancelledToken_DoesNotCrash()
    {
        ...
        var service2 = new AdvisorService(configMock.Object);
    ...
    public void Dispose_BeforeSetAdvisorModel_ThenSetAdvisorModel_DoesNotCrash()
    {
        ...
        // Act: Dispose 后调用 SetAdvisorModel 不应抛异常（Volatile.Read 检查 _disposed 跳过持久化）
        var act = () => service.SetAdvisorModel("model");
        act.Should().NotThrow();

...
public sealed partial class PlanApprovalMessageRouter : ServiceEntity {
    ...
    private Task _pendingTeammateRouting = Task.CompletedTask;
    ...
    public void StartTeammateRouting(string teammateAgentId) {
        _pendingTeammateRouting = Task.WhenAll(_pendingTeammateRouting, RouteTeammateResponsesAsync(teammateAgentId));
    }

    /// <summary>
    /// 等待所有 pending Teammate 路由任务完成 — 调用方可选 await 以确保路由落定
    /// </summary>
    public Task WaitForPendingTeammateRoutingAsync() => _pendingTeammateRouting;

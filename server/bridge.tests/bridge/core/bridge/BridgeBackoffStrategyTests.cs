...
public sealed class BridgeBackoffStrategyTests {
    ...
    public async Task Reset_WhenInErrorState_InvokesCallbackAndClearsState() {
        ...
        try { await sut.HandleErrorAsync(new HttpRequestException(), ct: cts.Token).ConfigureAwait(true); } catch (OperationCanceledException) { }
    ...
    public async Task HandleErrorAsync_SwitchingErrorType_ResetsOtherTrack() {
        ...
        // 再触发通用错误
        using (var cts = new CancellationTokenSource(TimeSpan.FromMilliseconds(50))) {
            try { await sut.HandleErrorAsync(new InvalidOperationException("boom"), ct: cts.Token).ConfigureAwait(true); } catch (OperationCanceledException) { }
        }
    ...
    public async Task HandleErrorAsync_NestedConnectionError_TreatedAsConnectionError() {
        ...
        try { await sut.HandleErrorAsync(outer, ct: cts.Token).ConfigureAwait(true); } catch (OperationCanceledException) { }

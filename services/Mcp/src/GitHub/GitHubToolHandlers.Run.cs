...
public partial class GitHubToolHandlers
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var summarized = SummarizeRunList(result.Body);
        var hasFailure = result.Body.Contains("\"conclusion\":\"failure\"", StringComparison.OrdinalIgnoreCase);
        return hasFailure ? Ok(summarized + RunListFailureHint) : Ok(summarized);
    }

    /// <summary>
    /// 精简 Actions Run 列表 JSON — 只保留关键字段，去掉冗余 URL，便于人类浏览和 AI 解析
    /// </summary>
    private static string SummarizeRunList(string json)
    {
        try
        {

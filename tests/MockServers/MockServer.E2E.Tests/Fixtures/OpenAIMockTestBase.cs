...
public abstract class OpenAIMockTestBase : IAsyncLifetime
{
    ...
    protected async Task SendInputAsync(string input, CancellationToken ct = default)
    {
        if (_cliProcess == null)
        {
            throw new InvalidOperationException("[E2E011] JoinCode 进程未启动。请先调用 StartJoinCodeAsync。");
        }
        ...
        CancellationToken ct = default)
    {
        if (_cliProcess == null)
        {
            throw new InvalidOperationException("[E2E012] JoinCode 进程未启动。请先调用 StartJoinCodeAsync。");
        }
    ...
    protected async Task<string> GetCurrentOutputAsync()
    {
        if (_cliProcess == null)
        {
            throw new InvalidOperationException("[E2E013] JoinCode 进程未启动。请先调用 StartJoinCodeAsync。");
        }
    ...
    protected async Task ClearOutputAsync()
    {
        if (_cliProcess == null)
        {
            throw new InvalidOperationException("[E2E014] JoinCode 进程未启动。请先调用 StartJoinCodeAsync。");
        }
